...
PowerModelsSE.jl is an extension package of PowerModels(Distribution).jl for Static Power System State Estimation. Currently, the package focusses on Distribution System State Estimation. This package is flexible design tool, enabling comparison and benchmarking of different State Estimation models. The flexibility rests in the easy integration of different power flow equations with different variable space, measurement numbers and types, measurement criteria, and more. Given its design-orientation, the package focuses on allowing the user easy model implementations, using the algebraic modeling toolbox [JuMP](https://jump.dev/JuMP.jl/stable/). JuMP provides an interface to off-the-shelf solvers that can solve the State Estimation, so the user can freely edit the model without the need to re-write the solving algorithm. Being the package a design facilitator, it is not the intention to provide the fastest state estimator. The solve time of off-the-shelf solvers is in general quite reasonable (although it depends on the model complexity). If faster calculations are crucial, these can be obtained with a customized algorithm, that the user can write afterwards, once the optimal State Estimation design has been chosen.
